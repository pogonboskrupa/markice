...
- **Markice po gazdinstvu** — tab Markice (samo farme, ne pčelari): izaberi farmu (nova farma se dodaje u tabu Gazdinstva), pa uporedi spisak grla na imanju sa spiskom vakcinisanih od veterinara. Oba spiska su tabele (markica, pol, vrsta — vakcinisani spisak i uz to do 5 kolona bolesti: Bruceloza/Enzotska leukoza/TBC/CMT/Antrax), popunjive ručno ili preko OCR slike; sačuvaj spisak trajno uz gazdinstvo. Ranije postojeće poređenje "cijele oblasti" odjednom (zajedno sa pretragom markice po broju i uvozom ugrađenog fajla) je ukinuto — sve ide po gazdinstvu.
- **Uvezi kompletan Excel fajl (jedan fajl, tri lista)** — u tabu Markice, jedan Excel fajl za jednog vlasnika (listovi Podaci/Stanje/Vakcinisano, poput stvarnih fajlova veterinarskih stanica) puni oba spiska odjednom; kolone se prepoznaju po nazivu u zaglavlju (ne po fiksnoj poziciji), pa radi i kad je raspored kolona drugačiji ili je markica zapisana u dvije kolone (Država + broj). Ako gazdinstvo nije izabrano, pronalazi ga automatski po šifri imanja ili vlasniku iz lista Podaci.
